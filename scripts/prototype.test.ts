...
describe('maintained standalone settings prototype', () => {
  it('generates deterministic offline HTML and rejects stale or missing artifacts without rewriting', async () => {
    ...
    expect(first).toContain('Live outline view')
    expect(first).toContain('Use samples')
    expect(first).not.toContain('Style changes apply immediately')
    ...
    await writeFile(output, first.replace('Live outline view', 'Stale preview'))
